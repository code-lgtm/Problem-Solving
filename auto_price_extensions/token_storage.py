  ...
  def get(self):
    ...
    try:
      ...
      try:
        ...
        logger.info('Using stored credentials from %s.', self._token_path())
        return credentials
      except google.auth.exceptions.RefreshError:
        logger.warning('The stored credentials in the file %s cannot be'
                       'refreshed, re-requesting access.', self._token_path())
        return None
...
def retrieve_client_config(config):
  ...
  if 'installed' not in client_config_json:
    logger.critical('Please read the note about OAuth2 client IDs in the '
                    'top-level README.')
    sys.exit(1)
