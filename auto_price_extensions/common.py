...
"""Common utils for the Content API for Shopping."""
...
def init(argv, doc, parents=None, sandbox=False):
  """A common initialization routine for the Content API.
  ...
  parser.add_argument(
      '--config_path',
      metavar='PATH',
      default=os.path.expanduser('~/shopping'),
      help='configuration directory for the Shopping')
  parser.add_argument(
      '--noconfig',
      action='store_true',
      help='run with no configuration directory')
  ...
  if not flags.noconfig:
    ...
    if not os.path.isfile(config_file):
      logger.info('Configuration file %s does not exist.', config_file)
      logger.info('Falling back to configuration based on authenticated user.')
    else:
  ...
  if _constants.ENDPOINT_ENV_VAR in os.environ:
    ...
    root_url = six.moves.urllib.parse.urljoin(
        ...
        '/')
    logger.info('Using non-standard root for API discovery: %s', root_url)
...
def retrieve_remaining_config_from_api(service, config):
  ...
  * websiteUrl
  ...
  if not account_ids:
    logger.critical('The currently authenticated user does not have access to '
  ...
  if 'merchantId' not in config:
    ...
    logger.info('Using Merchant Center %d.',
                config['merchantId'])
  ...
  for account_id in authinfo['accountIdentifiers']:
    ...
    if ('merchantId' in account_id and
      ...
      break
  if config['isMCA']:
    logger.info('Merchant Center %d is an MCA.', config['merchantId'])
  else:
    logger.info('Merchant Center %d is not an MCA.', config['merchantId'])
  ...
  if not config['websiteUrl']:
    logger.info('No website for Merchant Center %d.', config['merchantId'])
  else:
    logger.info('Website for Merchant Center %d: %s',
...
def check_mca(config, should_be_mca, msg=None):
  ...
  if should_be_mca != is_mca(config):
    if msg is not None:
      ...
      logger.critical('For this sample, you must%s use a multi-client account.',
  ...
  while True:
    try:
      ...
      if waited_time >= max_time:
        ...
        logger.warning('Request failed, trying again after %.2f seconds.',
                       sleep_time)
...
def get_logging_handler():
  """Provides a handler for all modules for consistent logging format.
